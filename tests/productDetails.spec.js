...
function productIdMustEndWith123 (array) {
  let condiction =''
  for (key in array) {
    if ( productDetails('Alcool gel', 'Máscara')[key]['details']['productId'].endsWith ('123') === false) {condiction = false; break;}
    if ( productDetails('Alcool gel', 'Máscara')[key]['details']['productId'].endsWith ('123') === true) {condiction = true;} 
  } return condiction;
}
...
describe('#productDetails', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    assert.notStrictEqual( productDetails('Alcool gel', 'Máscara')[0], productDetails('Alcool gel', 'Máscara')[0])
