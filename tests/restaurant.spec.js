...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const test = Object.keys(objetoRetornado.fetchMenu());
    assert.strictEqual(test.every((verifica) => verifica === 'food' || verifica === 'drinks'), true);// objetoRetornado.fetchMenu() // Retorno: { food: {}, drink: {}}
    // ```
    ...
    // ```
    const arrayConsumotion = objetoRetornado.consumption;
    objetoRetornado.order('coxinha');
    const analizar = arrayConsumotion.some((aux) => aux === 'coxinha');
    assert.strictEqual(analizar, true);
    ...
    const coxinhaInvst = objetoRetornado.consumption.filter((order) => order === 'coxinha').length;
    assert.strictEqual(coxinhaInvst > 1, true);
    // ```
