...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    const createMenuEmpty = createMenu(); // Retorno: { fetchMenu: () => {}, ... }
    assert.deepStrictEqual(typeof createMenuEmpty, 'object');
    assert.deepStrictEqual(typeof Object.values(createMenuEmpty)[0], 'function');
    // TESTE 2: Verifique que, dado que a função createMenu foi chamada com o objeto: `{ food: {}, drink: {} }`,
    // verifique que 'objetoRetornado.fetchMenu()' retorna um objeto cujas chaves são somente `food` e `drink`.
    const createMenuFoodAndDrink = createMenu({ food: {}, drink: {} }).fetchMenu();
    const expected = { food: {}, drink: {} };
    assert.deepStrictEqual(Object.keys(createMenuFoodAndDrink), ['food', 'drink']);
    // const objetoRetornado = createMenu({ food: {}, drink: {} });
    // objetoRetornado.fetchMenu() // Retorno: { food: {}, drink: {}}
    // ```
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    // ```
    assert.deepStrictEqual(createMenuFoodAndDrink, expected);
    ...
    assert.deepStrictEqual(createMenuEmpty.consumption, []);
    // const objetoRetornado = createMenu(objetoQualquer);
    ...
    // const createMenuCoxinha = createMenu().order('coxinha');
