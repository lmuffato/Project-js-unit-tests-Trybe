...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // ```
    assert.deepStrictEqual(createMenu({}).comsumption, [])
    ...
    test.order('coxinha');
    assert.deepStrictEqual(test.comsumption, ['coxinha']);
    ...
    test.order('sopa');
    test.order('sashimi');
    assert.deepStrictEqual(test.comsumption, ['coxinha', 'agua', 'sopa', 'sashimi']);
    ...
    test2.order('agua')
    test2.order('coxinha')
    assert.deepStrictEqual(test2.comsumption, ['coxinha', 'agua', 'coxinha'])
    ...
    // ```
    const newMenu = {
      food: { coxinha: 5.20, pastelP: 6.30, pastelG : 10.00},
      drink: { água: 3.00, refrigeranteLata: 4.30, sucoAgua: 6.00, sucoLeite: 7.50}
    }
    const test3 = createMenu(newMenu)
    test3.order('coxinha')
    test3.order('sucoAgua')
    test3.order('pastelG')
    test3.order('refrigeranteLata')
    assert.deepStrictEqual(test3.pay(), (5.20 + 6.00 + 10.00 + 4.30)*1.1)
