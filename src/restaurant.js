...
let createMenu = (obj) => {
  const objeto = {
    ...
    pay: () => {
      ...
      for (let foodOrDrink of arrayOfFoodAndDrinks) {
        for (let consumptionItem of consumptionItems) {
          if (foodOrDrink.includes(consumptionItem)) {
            sum += foodOrDrink[1];
          }
        }
      }
      return parseFloat(sum.toFixed(3), 10);
    },
  };
  return objeto;
};
