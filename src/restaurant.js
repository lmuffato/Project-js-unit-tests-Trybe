...
const consumo = (str) => {
  objCriado.consumption.push(str);
};

const createMenu = (obj) => {
  objCriado = { fetchMenu: () => obj,
    consumption: [],
    order: consumo,
    pay: () => {
      let total = 0;
      for (let index = 0; index < objCriado.consumption.length; index += 1) {
        const item = objCriado.consumption[index];
        if (typeof obj.food[item] === 'number') {
          total += obj.food[item];
        } else {
