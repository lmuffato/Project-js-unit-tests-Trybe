...
const createMenu = (objectReturn) => {
  const consumption = [];
  return {
    fetchMenu: () => objectReturn,
    consumption,
    order: (str) => {
      consumption.push(str);
    },
    pay: () => {
      const { food, drink } = objectReturn;
      const foodKeys = Object.keys(food); // ['coxinha'];
      const drinkKeys = Object.keys(drink); // ['agua'];
